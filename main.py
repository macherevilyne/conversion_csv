...
"""МЕНЯЕМ db_name НА АКТУАЛЬНЫЕ ДАННЫЕ ДЛЯ ПОДКЛЮЧЕНИЯ К СВОЕЙ БД"""
# Название базы данных, к которой вы хотите подключиться
db_name = "csv"

"""МЕНЯЕМ HOST, PORT, USER, PASSWORD НА АКТУАЛЬНЫЕ ДАННЫЕ ДЛЯ ПОДКЛЮЧЕНИЯ К СВОЕЙ БД"""
...
"""НУЖНО В НАЗВАНИИ ТАБЛИЦЫ 22q2 ПОМЕНЯТЬ НА АКТУАЛЬНОЕ НАЗВАНИЕ ТАБЛИЦЫ В БД """
...
"""НУЖНО В ПЕРЕМЕННОЙ directory ПОМЕНЯТЬ НА АКТУАЛЬНОЕ ДИРЕКТОРИЮ(ПУТЬ) ОТКУДА БУДУТ ЧИТАТЬСЯ ФАЙЛЫ CSV """
